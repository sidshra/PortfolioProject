...
image_data = []
# print(soup)
# ad_container = soup.find(class_=lambda c: c and "ad-container" in c)
# if ad_container:
...
for img in soup.find_all('img', src=True):
    ...
    # headline = parent_title.get_text()
    parent_title = img.find_parent('h3',class_ = 'title')
    if parent_title:
        headline = parent_title.text.strip()
...
with open("image_links_with_headlines.html", "w") as file:
    ...
    for image_src, headline, link_href in image_data:
        ...
        file.write(f"<img src='{image_src}' alt='Image'>")
